...
class BusinessTeamRequest extends FormRequest
{
    ...
    public function rules()
    {
        $validates = [
            ...
            'leader_uuid' => ['required', 'integer', Rule::exists('user_profiles', 'uuid')->whereNull('deleted_at')],
            'parent_team_uuid' => ['integer', Rule::exists('teams', 'uuid')->whereNull('deleted_at')],
            'team_member_uuids' => ['required', 'array', 'min:1'],
            'team_member_uuids.*' => ['required', 'integer', 'min:1', Rule::exists('user_profiles', 'uuid')->whereNull('deleted_at')]
        ];
