...
class FooterTemplateController extends AbstractRestAPIController
{
    ...
    {
        if (!Gate::allows('permission', 'Footer Template')) {
            return $this->sendJsonResponse(false, 'You need to buy platform/add-on', ['data' => $this->getPlatformByPermission('Footer Template')], 403);
        }
        $model = $this->service->create(array_merge($request->except('active_by_uuid'), [
            'user_uuid' => auth()->userId(),
            'app_id' => auth()->appId(),
